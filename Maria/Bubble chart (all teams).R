...
server <- function(input, output) {
  ...
  output$bubbleRace <- renderPlotly({
    ...
    all_teams <- unique(filtered$Team)
    ...
    # Generate colors dynamically for all teams
    team_colors <- viridisLite::viridis(n = length(all_teams))
    ...
    plot_ly(
      ...
      layout(
        title = "Olympic Medals by Team Over Time (All Teams, Every 4 Years)",
